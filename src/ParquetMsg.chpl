module ParquetMsg {
  ...
  proc writeMultiColParquet(filename: string, col_names: [] string, 
    ...
    coforall (loc, idx) in zip(targetLocales, filenames.domain) do on loc {
      ...
      var locSize: int = 0;
      forall column in sym_names with (+ reduce locSize) {
        ...
        if (entry.isAssignableTo(SymbolEntryType.SegStringSymEntry)) {
          ...
          locSize += + reduce lens[locDom];
        }
      }

      var str_vals: [0..#locSize] uint(8);
      var str_idx: int = 0;
      for (i, column) in zip(0..#ncols, sym_names) {
        ...
        select entryDtype {
          ...
          when DType.Bool {
            ...
            var valIdxRange = startValIdx..endValIdx;
            ref olda = ss.values.a;
            var start_idx = str_idx;
            forall (newIdx, valIdx) in zip(start_idx..#valIdxRange.size, valIdxRange) with (var agg = newSrcAggregator(uint(8))) {
              // Copy the remote value at index position valIdx to our local array
              agg.copy(str_vals[newIdx], olda[valIdx]); // in SrcAgg, the Right Hand Side is REMOTE
            }
            str_idx += valIdxRange.size;
            ptrList[i] = c_ptrTo(str_vals[start_idx]): c_void_ptr;
